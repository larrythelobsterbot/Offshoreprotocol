...
import type { CoinglassHeatmapLevel } from '../types';
...
export class CoinglassFeed extends EventEmitter {
  ...
  private async poll() {
    try {
        ...
        for (const entry of data) {
          if (entry.price && entry.liqValue) {
            levels.push({
              ...
              side: entry.side || (entry.price > 0 ? 'long' : 'short'),
            });
          }
        }
      } else if (data?.longs && data?.shorts) {
        // Alternative format with separated sides
        for (const entry of data.longs) {
          levels.push({
            price: parseFloat(entry.price || entry[0]),
            totalValue: parseFloat(entry.value || entry[1]),
            side: 'long',
          });
        }
        for (const entry of data.shorts) {
          levels.push({
            price: parseFloat(entry.price || entry[0]),
            totalValue: parseFloat(entry.value || entry[1]),
            side: 'short',
      ...
      logger.error({ err: err.message }, '[Coinglass] Poll error');
    }
  }
}
